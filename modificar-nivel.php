...
$el_nivel = $_GET['id_nivel'];


/******************* SQL PARA DESPLEGAR LOS USUARIOS QUE ESTAN EN EL SISTEMA *************/
$query_DatosOficios = sprintf("SELECT * FROM niveles  WHERE id_nivel= $el_nivel ORDER BY id_nivel DESC " );
...
$query_DatosOficios1 = sprintf("SELECT * FROM niveles ORDER BY id_nivel DESC " );
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "nuevonivel")) {
    ...
    $updateSQL= sprintf("UPDATE niveles SET descripcion=%s, autorizado=%s WHERE id_nivel= $el_nivel",
