...
$query_DatosOficios = sprintf("SELECT * FROM info_oficios WHERE (tipo_oficio=1 OR tipo_oficio=2) AND id_estado = 1 ORDER BY info_oficios.oficio_id DESC " );
...
$usuario_autorizado = GetSQLValueString(obtenerUsuarioAutorizadoEliminar1($_SESSION ['reservas_UserId']), "int"); 
$usuario_autorizado_ver = GetSQLValueString(obtenerUsuarioAutorizadoVer($_SESSION ['reservas_UserId']), "int");

/************************ WCG1 LLAMADO A FUNCION PARA CORROBORAR JEFATURA ****************/
$usuario_Jefatura = GetSQLValueString(obtenerJefatura($_SESSION ['reservas_UserId']), "int");
/************************ WCG1 FIN LLAMADO A FUNCION PARA CORROBORAR JEFATURA ****************/
  ...
  <body class="hold-transition skin-blue sidebar-mini">
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    <thead>
                      <tr>
                        ...
                        <th>Fecha</th>                    
                        <th>Año</th>
                      <!-- WCG1 AGREGAMOS LA COLUMNA -->  
                          <?php     
                    if ( $el_usuario == $usuario_Jefatura)
                        {  ?>
                      <th>Jefatura</th>
                      <?php } 

                        else echo "";?>
                     <!-- WCG1 FIN DE AGREGAMOS LA COLUMNA --> 
                    ...
                    if ( $el_usuario == $usuario_autorizado_ver)
                     ...
                     <?php 
                      if ( $el_usuario == $usuario_autorizado)
                        {  ?>
                        <th>Eliminar</th>
                        <?php } else echo "";?>
    ...
    //AQUI ES DONDE SE SACAN LOS DATOS, SE COMPRUEBA QUE HAY RESULTADOS
              ...
              ?>
                    ...
                    if ( $el_usuario == $usuario_Jefatura)
                        {  ?>
                        <td>  <a href="detalle_oficio_in.php?oficio_id=<?php echo $row_DatosOficios["oficio_id"];?>" class="btn btn-warning" role="button">Ver Detalle</a></td>
                        <?php } else echo "";?>
                        <!-- WCG1 FIN DE AGREGAMOS LA COLUMNA -->
...
<?php     
                    if ( $el_usuario == $usuario_autorizado_ver)
                        {  ?>
                        <td> 
                        <a class="btn btn-primary view-pdf" data-asunto-in="<?php echo $row_DatosOficios["asunto"];?>" data-id-in="<?php echo $row_DatosOficios["oficio_id2"];?>" data-numero-oficio-in="<?php echo $row_DatosOficios["no_oficio"];?>" data-fecha-in="<?php echo $row_DatosOficios["fecha"];?>" href="imagenes/oficios_in/<?php echo $row_DatosOficios["imagen"];?>">Ver PDF</a>        
...
<?php } else echo "";?>
                      ...
                      if ( $el_usuario == $usuario_autorizado)
                        ...
                        <a href="borrar_oficio_ingresado.php?oficio_id=<?php echo $row_DatosOficios["oficio_id"];?>"<?php  ?> <i class="glyphicon glyphicon-remove"></i> </td>
